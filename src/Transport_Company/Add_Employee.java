						...
						if (password.equals(password_1)) {
							...
							try {
								...
								if (com.if_branch_exists(branchId)) {
									com.insert_employee(id, name, address, mob, email, branchId,
											encryptor.encryptPassword(password));
									JOptionPane.showMessageDialog(null,
											"Successfully Added " + name + " as an Employee");
									dispose();
								} else {
									JOptionPane.showMessageDialog(null, "Error", "Branch Doesn't Exist",
											JOptionPane.ERROR_MESSAGE);
								}
							} catch (Exception ex) {
								ex.printStackTrace();
								JOptionPane.showMessageDialog(null, "Error", "Connection not possible",
										JOptionPane.ERROR_MESSAGE);
							}
						} else {
							JOptionPane.showMessageDialog(null, "Error", "Passowrds didn't match",
									JOptionPane.ERROR_MESSAGE);
						}
					}
				});
